...
def visual_deg_to_pix(deg, dist, physical_screen_size, resolution):
    ...
    """
    visual_angle_degrees = (2 * atan(physical_screen_size[0] / (2 * dist))) * (
            180 / pi)
...
DISTANCE_BETWEEN_IMAGES = [
    2]  # [0.9, 1.5, 0.9]  # percentage of image size between two images
...
TARGET_CATEGORIES = []  # fill category names that are targets

####################### automatically determined parameters ########################

TARGET_IMAGE_SIZE = [round(SCREEN_RESOLUTION[0] * 0.75),
                     round(SCREEN_RESOLUTION[1] * 0.75)]

IMAGES_SIZE = visual_deg_to_pix(VIS_DEG_FOR_STIM, SCREEN_DISTANCE,
                                PHYSICAL_SCREEN_SIZE, SCREEN_RESOLUTION)

# the amount of jitter to add to x,y location, relative to image size
LOCATION_JITTER = [int(ceil((IMAGES_SIZE * dist) / 10)) for dist in
                   DISTANCE_BETWEEN_IMAGES]  # 4

IMAGES_ON_X = [TARGET_IMAGE_SIZE[0] // round(IMAGES_SIZE * dist) for dist in
               DISTANCE_BETWEEN_IMAGES]
print(f"IMAGES_ON_X: {IMAGES_ON_X}")
IMAGES_ON_Y = [TARGET_IMAGE_SIZE[1] // round((IMAGES_SIZE * dist)) for dist in
               DISTANCE_BETWEEN_IMAGES]
print(f"IMAGES_ON_Y: {IMAGES_ON_Y}")
ALL_IMAGES_NUM = [IMAGES_ON_X[i] * IMAGES_ON_Y[i] for i in
                  range(len(IMAGES_ON_X))]

NUM_OF_NO_TARGET_IMAGES = [
    round((1 - expectancy) * NUM_OF_IMAGES_TO_CREATE_PER_CONDITION) for
    expectancy in
    TARGETS_EXPECTATION_PERCENTAGE]
OVERALL_X_WIDTH = [IMAGES_SIZE * DISTANCE_BETWEEN_IMAGES[i] * IMAGES_ON_X[i] for
                   i in range(len(IMAGES_ON_X))]
OVERALL_Y_HEIGHT = [IMAGES_SIZE * DISTANCE_BETWEEN_IMAGES[i] * IMAGES_ON_Y[i]
                    for i in range(len(IMAGES_ON_Y))]
X_OFFSET = [((SCREEN_RESOLUTION[0] - x) // 2) - (IMAGES_SIZE // 2) for x in
            OVERALL_X_WIDTH]
Y_OFFSET = [((SCREEN_RESOLUTION[1] - y) // 2) - (IMAGES_SIZE // 2) for y in
            OVERALL_Y_HEIGHT]
...
def _add_info_to_image(im, image_name):
    ...
    sub_cat_end_index = cat_end_index + 1 + image_name[cat_end_index + 1:].find(
        '_')
    im.info[RELEVANT_CATEGORY_INFO_NAME2] = image_name[
                                            cat_end_index + 1:sub_cat_end_index]
    if (image_name.count('_') > 2 and 'target' not in image_name) \
            or (image_name.count('_') > 3 and 'target' in image_name):
        sub_sub_cat_end_index = sub_cat_end_index + 1 + image_name[
                                                        sub_cat_end_index + 1:].find(
            '_')
        im.info[RELEVANT_CATEGORY_INFO_NAME1] = image_name[
                                                sub_cat_end_index + 1:sub_sub_cat_end_index]
    im.info["category_key"] = im.info[RELEVANT_CATEGORY_INFO_NAME2] + '_' + \
                              im.info[RELEVANT_CATEGORY_INFO_NAME1] \
        if RELEVANT_CATEGORY_INFO_NAME1 in im.info else im.info[
        RELEVANT_CATEGORY_INFO_NAME2]
    ...
    while failure:
        try:
            ...
            for i in range(len(TARGETS_EXPECTATION_PERCENTAGE)):
                ...
                mkdir(osp_join('generated_images' + str(try_idx),
                               cond_str + '_Color'))
                mkdir(osp_join('generated_images' + str(try_idx),
...
def _equalize_luminance(im, new_mean):
    ...
    yiq = rgb2yiq(rgb)
    cur_mean = np.mean(
        yiq[non_transparent_indices, 0])  # disregard pixels with alpha=0
    yiq[non_transparent_indices, 0] -= (cur_mean - new_mean)
    rgb = np.round(
        yiq2rgb(yiq) * 255)  # convert back to RGB nad non-normalized image
...
def _location_is_on_edge(location):
    ...
    """
    return (location[0] > (TARGET_IMAGE_SIZE[0] - (
            NUM_OF_IMAGES_THAT_ARE_CONSIDERED_EDGE * IMAGES_SIZE)) or
            location[
                0] < NUM_OF_IMAGES_THAT_ARE_CONSIDERED_EDGE * IMAGES_SIZE or
            location[1] > (TARGET_IMAGE_SIZE[1] - (
                    NUM_OF_IMAGES_THAT_ARE_CONSIDERED_EDGE * IMAGES_SIZE)) or
            location[1] < NUM_OF_IMAGES_THAT_ARE_CONSIDERED_EDGE * IMAGES_SIZE)


def _rotate_image(image, angle,
                  representation):  # representation - RGB or GRAYSCALE
    image = ImageOps.expand(image, image.size * 2,
                            (0, 0) if representation else (0, 0, 0, 0))
...
def _get_locations(image_list, cond_index):
    ...
    jitters = np.random.randint(-LOCATION_JITTER[cond_index],
                                LOCATION_JITTER[cond_index],
                                ALL_IMAGES_NUM[cond_index] * 2).reshape(
        (2, IMAGES_ON_Y[cond_index], IMAGES_ON_X[cond_index]))
    # create locations grid
    x, y = np.meshgrid(
        np.round(np.arange(IMAGES_ON_X[cond_index]) * (
                IMAGES_SIZE * DISTANCE_BETWEEN_IMAGES[cond_index])),
        np.round(np.arange(IMAGES_ON_Y[cond_index]) * (
                IMAGES_SIZE * DISTANCE_BETWEEN_IMAGES[cond_index])))

    # calculate offsets to x,y and center image locations
    x_offset = round(
        TARGET_IMAGE_SIZE[0] - (IMAGES_ON_X[cond_index] * IMAGES_SIZE *
                                DISTANCE_BETWEEN_IMAGES[cond_index]))
    y_offset = round(
        TARGET_IMAGE_SIZE[1] - (IMAGES_ON_Y[cond_index] * IMAGES_SIZE *
                                DISTANCE_BETWEEN_IMAGES[cond_index]))
    x += x_offset
    y += y_offset
    # add jitters
...
def _paste_images(image_list, bw_image_list, locations, final_im,
                  final_im_no_background, final_im_bw,
    ...
    for cat_num in image_category_order:
        for i in range(len(image_list)):
            if image_list[i].info[
                "cat_num"] == cat_num and i not in used_indices:
    ...
    for i, loc in enumerate(locations):
        _rotate_and_paste_image(image_list_sorted[i], bw_image_list_sorted[i],
                                loc, angles[i], final_im, final_im_bw,
                                final_im_no_background)
        if image_list_sorted[i].info['target']:
            target_indices.append(i)
    for i in target_indices:
        _rotate_and_paste_image(image_list_sorted[i], bw_image_list_sorted[i],
                                locations[i], angles[i], final_im,
                                final_im_bw,
                                final_im_no_background)


def _rotate_and_paste_image(image, bw_image, loc, angle, final_im, final_im_bw,
                            final_im_no_background):
    ...
    final_im_bw.paste(cur_img_bw.convert('RGBA'), loc,
                      cur_img_bw.convert('RGBA'))
...
def _create_locations_xl(locations_list, images, try_idx, image_num,
    ...
    for i, location in enumerate(locations_list):
        category = images[i].info[RELEVANT_CATEGORY_INFO_NAME2] + '_' + \
                   images[i].info[RELEVANT_CATEGORY_INFO_NAME1] \
            if RELEVANT_CATEGORY_INFO_NAME1 in images[i].info else \
            images[i].info[RELEVANT_CATEGORY_INFO_NAME2]
...
def _save_images(final_image, final_image_bw, final_image_no_clutter,
    ...
    final_image.convert('RGB').save(osp_join(curdir, color_dir_path,
                                             "image_" + str(
                                                 image_num + 1) + ".png"),
                                    "PNG")
    ...
    final_image_bw.convert('LA').save(osp_join(curdir, bw_dir_path,
                                               "image_bw_" + str(
                                                   image_num + 1) + ".png"))
    targets_im_bw.convert('LA').save(osp_join(curdir, bw_dir_path,
                                              "targets_bw_" + str(
                                                  image_num + 1) + ".png"))
    targets_im.save(osp_join(curdir, color_dir_path,
                             "targets_" + str(image_num + 1) + ".png"))
    targets_im.save(osp_join(curdir, color_no_clutter_dir_path,
                             "targets_" + str(image_num + 1) + ".png"))


def _save_images_with_roi(final_images_with_roi,
                          final_images_no_clutter_with_roi,
                          final_images_bw_with_roi,
                          image_num, try_idx):
    cond_index = image_num // NUM_OF_IMAGES_TO_CREATE_PER_CONDITION
    cond_str = '_expectancy_' + str(
        TARGETS_EXPECTATION_PERCENTAGE[cond_index]) + "_clutter_" + str(
    ...
    for i, image in enumerate(final_images_with_roi):
        image.convert('RGB').save(
            osp_join(curdir, color_dir_path,
                     "image_" + str(image_num + 1) + "_with_roi_" + str(
                         i + 1) + ".png"), "PNG")
    for i, image in enumerate(final_images_no_clutter_with_roi):
        image.convert('RGB').save(osp_join(curdir, color_no_clutter_dir_path,
                                           "image_" + str(
                                               image_num + 1) + "_no_clutter_with_roi_" +
                                           str(i + 1) + ".png"), "PNG")
    for i, image in enumerate(final_images_bw_with_roi):
        image.convert('RGB').save(
            osp_join(curdir, bw_dir_path,
                     "image_bw_" + str(image_num + 1) + "_with_roi_" + str(
                         i + 1) + ".png"), "PNG")


def _generate_roi(final_images_list, location_list, color=ROI_COLOR,
...
def _generate_targets_images(targets):
    ...
    """
    targets_im_x_size, targets_im_y_size = sum(
        target.size[0] for target in targets), max(
        target.size[1] for target in targets)
    targets_im = Image.new('RGBA', (targets_im_x_size, targets_im_y_size),
                           color="gray")
    targets_im_bw = Image.new('LA', (targets_im_x_size, targets_im_y_size),
                              color="gray")
    x_offset = 0
...
def _choose_targets(targets, not_resized_targets):
    ...
    """
    all_targets = [[] for _ in TARGET_CATEGORIES]
    not_resized_all_targets = [[] for _ in TARGET_CATEGORIES]
    for i, target_category_name in enumerate(TARGET_CATEGORIES):
        for j, im in enumerate(targets):
            if im.info["category_key"] == target_category_name:
                all_targets[i].append(im)
                not_resized_all_targets[i].append(not_resized_targets[j])
    overall_target_num = int(NUM_OF_IMAGES_TO_CREATE_PER_CONDITION * len(
        TARGETS_EXPECTATION_PERCENTAGE) * NUM_OF_TARGETS_PER_IMAGE)

    target_num_per_category = [overall_target_num // len(TARGET_CATEGORIES) for
                               _ in all_targets]
    if np.sum(target_num_per_category) != overall_target_num:
        target_num_per_category[0] += overall_target_num - np.sum(
            target_num_per_category)
    for i in range(len(all_targets)):
        np.random.shuffle(all_targets[i])
        np.random.shuffle(not_resized_all_targets[i])
    inds = [np.random.choice(np.arange(len(all_targets[i])),
                             target_num_per_category[i], False) for i in
            range(len(all_targets))]

    chosen_targets = [[all_targets[i][ind] for ind in inds[i]] for i in
                      range(len(all_targets))]
    not_resized_chosen_targets = [
        [not_resized_all_targets[i][ind] for ind in inds[i]] for i
        in range(len(all_targets))]

    # TODO: create an array of image lists, each one containing a single target
    # chosen_indices = np.random.choice(np.arange(len(targets)),
    #                                   NUM_OF_IMAGES_TO_CREATE_PER_CONDITION * len(
    #                                       TARGETS_EXPECTATION_PERCENTAGE) * NUM_OF_TARGETS_PER_IMAGE,
    #                                   TARGETS_REPETITION)
    # targets_pairings = [chosen_indices[i::NUM_OF_TARGETS_PER_IMAGE] for i in
    #                     range(NUM_OF_TARGETS_PER_IMAGE)]
...
def _choose_image_lists(chosen_targets, distractor_images_dict):
    ...
    num_of_images_per_category = [ceil(image_num / num_of_categories) for
                                  image_num in ALL_IMAGES_NUM]
    print(f"number of images per category: {num_of_images_per_category}")
    images_list = []
    for i, targets in enumerate(
            chosen_targets):  # go through all target images of each image to create
        ...
        add_targets = (i % NUM_OF_IMAGES_TO_CREATE_PER_CONDITION) >= \
                      NUM_OF_NO_TARGET_IMAGES[cond_index]
        if add_targets:
        ...
        for category in distractor_images_dict:  # choose and add distractors
            ...
            size = num_of_images_per_category[cond_index] - (
                0 if category not in chosen_targets_categories else
                chosen_targets_categories[category])
            cur_image_list += list(np.random.choice(a, size=size,
                                                    replace=True if size > len(
                                                        a) else False))
        if add_targets:
...
def generate_image(args):
    ...
    locations = _get_locations(image_list,
                               image_num_entry // NUM_OF_IMAGES_TO_CREATE_PER_CONDITION)

    # create final images
    final_im_bw = Image.new('RGBA', TARGET_IMAGE_SIZE, color="gray")
    final_im = Image.open(BACKGROUND_IMAGE).convert('RGBA').resize(
        TARGET_IMAGE_SIZE, Image.ANTIALIAS)
    final_im_no_background = Image.new('RGBA', TARGET_IMAGE_SIZE, color="gray")

    # paste images to final images
    _paste_images(image_list, bw_image_list, locations, final_im,
                  final_im_no_background, final_im_bw,
                  image_category_order)

    targets_im, targets_im_bw = _generate_targets_images(not_resized_targets)

    _save_images(final_im, final_im_bw, final_im_no_background, targets_im,
                 targets_im_bw, image_num_entry, try_idx)

    _create_locations_xl(locations, image_list, try_idx, image_num_entry,
                         categories_dict)
    # create ROI's
    target_locations = [loc for i, loc in enumerate(locations) if
                        image_list[i].info['target']]
    ...
    _save_images_with_roi(final_image_with_roi, final_image_no_clutter_with_roi,
                          final_image_bw_with_roi,
...
def main():
    ...
    distractor_images_dict, targets, not_resized_targets, categories_dict = _load_images(
        _choose_image_directory())
    # print("equalizing images...")
    ...
    chosen_targets, not_resized_chosen_targets = _choose_targets(targets,
                                                                 not_resized_targets)
    ...
    if PARALLEL:
        ...
        pool.map(generate_image,
                 [(try_idx, num, image_lists[num],
                   not_resized_chosen_targets[num], categories_dict,
                   perms[num, :]) for
                  num in
                  range(NUM_OF_IMAGES_TO_CREATE_PER_CONDITION * len(
                      TARGETS_EXPECTATION_PERCENTAGE))])
    else:
        print("starting generation of images...")
        for num in range(NUM_OF_IMAGES_TO_CREATE_PER_CONDITION * len(
                TARGETS_EXPECTATION_PERCENTAGE)):
            generate_image(
                args=(
                    try_idx, num, image_lists[num],
                    not_resized_chosen_targets[num],
                    categories_dict, perms[num, :]))
